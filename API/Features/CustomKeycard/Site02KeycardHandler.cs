...
public class Site02KeycardHandler
{
    ...
    private static readonly string Site02CommandUsage = EasyArgs.Build()
        .NewLine()
    ...
    public static bool Site02KeycardCreator(ArraySegment<string> uncutArguments, ExPlayer playerToGiveCard, ICommandSender commandSender)
    {
        if (uncutArguments.Count < 3) // arg2, itemName
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Orange("Formatting information:").NewLine().Blue("Use underscores for spaces.").Done() + Site02CommandUsage);
            return false;
        }
        ...
        if (uncutArguments.Count < 4) // arg3, containmentLevel
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Orange("Formatting information:").NewLine().Blue("Use numbers 0-5.").Done() + Site02CommandUsage);
            return false;
        }
        ...
        if (!argument3.GetCustomKeycardLevel(out var containmentLevel, out var cKeycardContainmentLevel) || containmentLevel == null || cKeycardContainmentLevel == null)
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Blue("Use numbers 0-5.").Done() + Site02CommandUsage);
            return false;
        }
        ...
        switch (uncutArguments.Count)
        {
            ...
            case < 9:
                commandSender.Respond(EasyArgs.Build()
                    ...
                    .NewLine().Orange("Formatting information:")
                    .NewLine().Blue("Use \"_\" for spaces! To use underscores, use \"\\_\"!").Done() + Site02CommandUsage);
                return false;
            // arg9, label
            case < 10:
                commandSender.Respond(EasyArgs.Build()
        ...
        var site02Keycard = KeycardItem.CreateCustomKeycardSite02(
            playerToGiveCard,
            itemName,
            holderName,
            label,
        ...
        if (site02Keycard == null)
        {
            commandSender.Respond(
                ...
                + EasyArgs.Build()
                    ...
                    .Space()
                    .Done()
                + $"{(playerToGiveCard.IsScp ? "<color=green>True</color>" : "<color=red>False</color>")}" + Site02CommandUsage);
            return false;
        }
        ...
        KeycardHandler.Instance.GiveCustomKeycard(playerToGiveCard, site02Keycard, grppKeycard, commandSender);
