...
public static class ManagementKeycardHandler
{
    ...
    private static readonly string ManagementCommandUsage = EasyArgs.Build()
        .NewLine()
    ...
    public static bool ManagementKeycardCreator(ArraySegment<string> uncutArguments, ExPlayer playerToGiveCard, ICommandSender commandSender)
    {
        if (uncutArguments.Count < 3) // arg2, itemName
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Orange("Formatting information:").NewLine().Blue("Use underscores for spaces.").Done() + ManagementCommandUsage);
            return false;
        }
        ...
        if (uncutArguments.Count < 4) // arg3, containmentLevel
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Orange("Formatting information:").NewLine().Blue("Use numbers 0-5.").Done() + ManagementCommandUsage);
            return false;
        }
        ...
        if (!argument3.GetCustomKeycardLevel(out var containmentLevel, out var cKeycardContainmentLevel) || containmentLevel == null || cKeycardContainmentLevel == null)
        {
            commandSender.Respond(EasyArgs.Build()
                ...
                .NewLine().Blue("Use numbers 0-5.").Done() + ManagementCommandUsage);
            return false;
        }
        ...
        switch (uncutArguments.Count)
        {
            ...
            case < 9:
                commandSender.Respond(EasyArgs.Build()
                    ...
                    .NewLine().Orange("Formatting information:")
                    .NewLine().Blue("Use \"_\" for spaces! To use underscores, use \"\\_\"!").Done() + ManagementCommandUsage);
                return false;
            // arg9, label
            case < 10:
                commandSender.Respond(EasyArgs.Build()
        ...
        var managementKeycard = KeycardItem.CreateCustomKeycardManagement(
            playerToGiveCard,
            itemName,
            label,
        ...
        if (managementKeycard == null)
        {
            commandSender.Respond(
                ...
                + EasyArgs.Build()
                    ...
                    .Space()
                    .Done()
                + $"{(playerToGiveCard.IsScp ? "<color=green>True</color>" : "<color=red>False</color>")}" + ManagementCommandUsage);
            return false;
        }
        ...
        KeycardHandler.Instance.GiveCustomKeycard(playerToGiveCard, managementKeycard, grppKeycard, commandSender);
